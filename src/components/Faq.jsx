...
function FAQ() {
  ...
  return (
        ...
        <div className="text-white w-3/5 m-auto my-[4rem]  ">
      ...
      {faqs.map((faq, index) => (
        <div key={index} className=" py-4">
          ...
          {activeIndex === index && (
            <p className="mt-2 px-4 text-gray-500 p-[1.5rem] border border-white bg-gray-300 bg-opacity-5 backdrop-blur-md rounded-md">{faq.answer}</p>
          )}
        </div>
      ))}
    </div>
